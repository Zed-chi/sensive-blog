...
def index(request):    
    posts = Post.objects.prefetch_related("author", Prefetch("tags", queryset=Tag.objects.popular()))
...
def post_detail(request, slug):
    posts = Post.objects.popular().prefetch_related("author")
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all().popular()[:20].fetch_with_comments_count()
